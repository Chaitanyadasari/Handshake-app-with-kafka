...
class Jobs extends React.Component {
  ...
  constructor() {
    super();
    this.state = { jobs: [], jobsSearch: [], selectedJob: '', companyName: '', selectedFile: null};
  }
      ...
      .then(res => {
        if (res.status === 200) {
          this.setState({ jobs: res.data }, () => {
            ...
            this.setState({selectedJob: res.data[0]})
          });
          this.setState({jobsSearch: res.data })
        }
      })
      .catch(err => {
        console.log(err);
  ...
  onSelectJob = (job, companyName) => {
    this.setState({selectedJob: job, companyName: companyName});
  }

  onClickUpload = () => {

    const student_id = sessionStorage.getItem("userId");
    const jobId = this.state.selectedJob._id
    const data = {student_id: student_id}
    axios.put(`http://localhost:3000/applications/apply/${jobId}`, data, {headers: {'Content-Type': 'application/json'}})
          .then(res => {
            if (res.status === 200) {
                alert("Applied")
                // this.setState({ redirect: <Redirect to='/student/registered/event' /> });
            }
          })
          .catch(err => {
            console.log(err);
    ...
    //  //   if (res.status === 200) {
    //       axios
    //         .post('http://localhost:3000/applications/apply/${id}', {
      ...
      <div>
        ...
        <div style={{marginLeft: '30px', marginTop: '20px', marginRight: '', float: 'left', width: '43%'}}>
            <div className='ui raised segment'>
              ...
              <div>
                Job category: {this.state.selectedJob.category}
              </div>
              <div>
                Description: {this.state.selectedJob.jobDescription}
              </div>
              <div>
                Job Requirements: {this.state.selectedJob.jobRequirements}
              </div>
              <div>
